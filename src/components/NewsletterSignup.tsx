...
const NewsletterSignup = ({ variant = "inline", backgroundImage }: NewsletterSignupProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await subscribe.mutateAsync({ email, name, source: variant });
      toast({ title: "You're on the list!", description: `We'll send new travel stories to ${email}.` });
      setEmail("");
      setName("");
    } catch (error) {
      const duplicate = (error as { code?: string })?.code === "23505";
      toast({
        title: duplicate ? "Already subscribed" : "Something went wrong",
        description: duplicate
          ? "That email address is already on our list."
